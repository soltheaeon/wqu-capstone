...
class SmartBetaStrategies(QCAlgorithm):
    ...
    def Initialize(self) -> None:
        ...
        self.SetStartDate(RunConfig.StartDate.year,RunConfig.StartDate.month, RunConfig.StartDate.day)  # Note: initial model will be trained with 1 year data
        ...
        if RunConfig.UseMLForRebalancing:
            self.Schedule.On(self.DateRules.MonthStart(benchmark.Symbol), self.TimeRules.At(8,0), self.RebalanceOnML)
        else:
            self.Schedule.On(self.DateRules.MonthStart(benchmark.Symbol), self.TimeRules.At(8,0), self.Rebalance)
    ...
    def OnWarmUpFinished(self) -> None:
        self.Log("Warm-Up finished. Algorithm Ready")
    ...
    def FineSelectionFunction(self, fine: List[FineFundamental]) -> List[Symbol]:
        ...
        top = sorted(fine, key = lambda x: x.MarketCap, reverse=True)
        ...
        for x in top:
            ...
            x.OperationRatios.TotalDebtEquityRatio.Value, x.EarningReports.BasicEPS.Value, x.MarketCap, 
